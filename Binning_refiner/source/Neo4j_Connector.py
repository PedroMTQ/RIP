...
from source.util import get_instance_type, is_reversible,uniform_sign,find_sign,regex_escape

import time
from sys import platform as pc_platform
from math import ceil
from copy import copy


class Neo4j_Connector():
    def __init__(self,uri=None,username=None,password=None,try_limit=30,db_name=None):
        # Database Credentials
        if pc_platform.startswith('win'):
            # this depends on installation path of marvinsuite!
            port = '7687'
        elif 'linux' in pc_platform:
            port = '7687'
        if not uri: uri = 'bolt://localhost:'+port
        if not username: username = 'neo4j'
        if not password: password = 'neo4j'
    ...
    def create_constraints(self):
        return [' CONSTRAINT ON (Identifier:ID) ASSERT Identifier.ID IS UNIQUE',
                     ...
                     ' CONSTRAINT ON (Aminoacid_sequence:ES_ID) ASSERT Aminoacid_sequence.ES_ID IS UNIQUE',
                     ' CONSTRAINT ON (Nucleotide_sequence:ES_ID) ASSERT Nucleotide_sequence.ES_ID IS UNIQUE',
    ...
    def connect_to_neo4j(self):
        ...
        if self.first_connection:
            try:
                print('Connecting to Neo4j database "',self.db_name,'" with '+ get_instance_type(self))
                self.bolt_connections = 0
    ...
    def reset_db(self,force_reset=False,delete_all_node_type=None):
        if delete_all_node_type:           reset_db_command = 'match (n)-->(i) where NOT n:'+delete_all_node_type+' AND NOT i:'+delete_all_node_type+' return n,i'
        else: reset_db_command = 'MATCH (n) ' \
                                 'WHERE NOT n:Web_Connector ' \
                                 'AND NOT n:Broken_Link ' \
                                 'AND NOT n:Proxy ' \
                                 'AND NOT n:User_Agent ' \
                                 'DETACH DELETE n'
        if force_reset:
            self.run_command_neo4j(reset_db_command)
            return
        remove_choice = input('Are you sure you want to reset the database, except the compounds?  \nThis is not reversible! "Y" to reset\n')
        if remove_choice == 'Y':
            reset_db_command = 'MATCH (n) DETACH DELETE n'
            self.run_command_neo4j(reset_db_command)
            print('DATABASE RESET')
        else: print('DATABASE NOT RESET')
...
if __name__ == '__main__':
    ...
    username = 'neo4j'
    password = 'neo4j'
    neo4j_driver = Neo4j_Connector(uri, username, password,db_name='neo4j')
    neo4j_driver.connect_to_neo4j()
    res=neo4j_driver.run_command_neo4j('match (n) return id(n)')
    for i in res:
        print(i)
    for i in res:
        print(i)
